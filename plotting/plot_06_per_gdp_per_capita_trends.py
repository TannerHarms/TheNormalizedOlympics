"""
Plot medals per GDP per capita trends over time.

Shows line plots of the top and bottom countries by medals per (GDP/Population)
over the specified year range, tracking how their normalized performance evolves.
"""
...
def plot_per_gdp_per_capita_trends():
    """Create line plots showing per GDP per capita medal trends over time."""
    ...
    for season, df in [('Summer', summer_df), ('Winter', winter_df)]:
        ...
        # Filter to countries with both GDP and Population data
        df_filtered = df_filtered[(df_filtered['GDP'].notna()) & (df_filtered['Population'].notna())].copy()
        
        # Calculate GDP per capita (in USD)
        df_filtered['GDP_Per_Capita'] = df_filtered['GDP'] / df_filtered['Population']
        
        # Calculate medals per GDP per capita (medals per 1 unit of GDP per capita)
        df_filtered['Medals_Per_GDP_Per_Capita'] = df_filtered['Total'] / df_filtered['GDP_Per_Capita'] * 1000
        ...
        # Find top and bottom countries by average performance
        country_avg = df_recent.groupby('Country')['Medals_Per_GDP_Per_Capita'].mean()
        ...
        print(f"Top {TOP_N_COUNTRIES} countries by avg medals per GDP per capita:")
        for country in top_countries:
            avg_val = country_avg[country]
            print(f"  {country}: {avg_val:.3f}")
        ...
        for idx, country in enumerate(top_countries):
            ...
            if len(country_data) > 0:
                ...
                ax.plot(
                    country_data['Year'],
                    country_data['Medals_Per_GDP_Per_Capita'],
                    marker='o',
                    linewidth=2.5,
                    markersize=8,
        ...
        ax.set_ylabel('Medals per $1000 GDP per Capita', fontsize=16, weight='bold')
        ax.set_title(
            f"Top {TOP_N_COUNTRIES} Countries: Medals per GDP per Capita Over Time\n"
        ...
        # Save top plot
        filename = f'{season.lower()}_per_gdp_per_capita_trends_top_{TOP_N_COUNTRIES}.png'
        ...
        ax.set_ylabel('Medals per $1000 GDP per Capita', fontsize=16, weight='bold')
        ax.set_title(
            f"Bottom {TOP_N_COUNTRIES} Countries: Medals per GDP per Capita Over Time\n"
            f"{season} Olympics ({START_YEAR}-{END_YEAR})",
            fontsize=18,
            weight='bold',
            pad=20
        ...
        # Save bottom plot
        filename = f'{season.lower()}_per_gdp_per_capita_trends_bottom_{TOP_N_COUNTRIES}.png'
        output_file = PLOTS_DIR / filename
        save_plot(fig, output_file)
        plt.close()
        print(f"✓ Saved: {output_file}")
...
if __name__ == '__main__':
    plot_per_gdp_per_capita_trends()
