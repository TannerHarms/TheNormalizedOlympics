"""
Plot medals normalized by GDP for medal-winning countries.

Shows:
- Top 10 countries by medals per billion USD GDP
- Bottom 10 countries by medals per billion USD GDP
...
def plot_normalized_medals(df, metric_col, metric_name, season, year, top_n=10):
    ...
    """
    # Filter to most recent year with medals and GDP data
    recent = df[df['Year'] == year].copy()
    recent = recent[recent['Total'] > 0]  # Only medal winners
    recent = recent[recent[metric_col].notna()]  # Only with GDP data
    ...
    for i, (val, total, gdp) in enumerate(zip(values_top, top['Total'].values, top['GDP'].values)):
        gdp_trillions = gdp / 1e12
        ax_top.text(val + val*0.02, i, f'{val:.3f} ({int(total)} medals, ${gdp_trillions:.1f}T)', 
    ...
    # Add values at end of bars
    for i, (val, total, gdp) in enumerate(zip(values_bottom, bottom['Total'].values, bottom['GDP'].values)):
        gdp_trillions = gdp / 1e12
        ax_bottom.text(val + val*0.02, i, f'{val:.3f} ({int(total)} medals, ${gdp_trillions:.1f}T)', 
                      va='center', fontsize=9)
...
def main():
    print("="*70)
    print("GDP-NORMALIZED MEDALS: TOP & BOTTOM PERFORMERS")
    print("="*70)
    print()
    ...
    # ===== SUMMER: PER GDP =====
    print(f"Summer {summer_year}: Medals per billion USD GDP")
    medal_winners = summer[(summer.Year==summer_year) & (summer.Total > 0)]
    with_gdp = medal_winners[medal_winners.Medals_per_billion_GDP.notna()]
    print(f"  Medal winners: {len(medal_winners)}")
    print(f"  With GDP data: {len(with_gdp)} ({100*len(with_gdp)/len(medal_winners):.0f}%)")
    ...
    if fig_top:
        save_plot(fig_top, PLOTS_DIR / f'summer_{summer_year}_per_GDP_TOP_{TOP_N}.png')
        plt.close(fig_top)
        print(f"  ✓ Saved: summer_{summer_year}_per_GDP_TOP_{TOP_N}.png")
    
    if fig_bottom:
        save_plot(fig_bottom, PLOTS_DIR / f'summer_{summer_year}_per_GDP_BOTTOM_{TOP_N}.png')
        plt.close(fig_bottom)
        print(f"  ✓ Saved: summer_{summer_year}_per_GDP_BOTTOM_{TOP_N}.png")
    
    print()
    
    # ===== WINTER: PER GDP =====
    print(f"Winter {winter_year}: Medals per billion USD GDP")
    ...
    if fig_top:
        save_plot(fig_top, PLOTS_DIR / f'winter_{winter_year}_per_GDP_TOP_{TOP_N}.png')
        plt.close(fig_top)
        print(f"  ✓ Saved: winter_{winter_year}_per_GDP_TOP_{TOP_N}.png")
    
    if fig_bottom:
        save_plot(fig_bottom, PLOTS_DIR / f'winter_{winter_year}_per_GDP_BOTTOM_{TOP_N}.png')
        plt.close(fig_bottom)
        print(f"  ✓ Saved: winter_{winter_year}_per_GDP_BOTTOM_{TOP_N}.png")
