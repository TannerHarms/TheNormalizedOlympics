"""
Plot medals per capita (normalized by population) for medal-winning countries.

Shows:
- Top 10 countries by medals per million population
- Bottom 10 countries by medals per million population
...
def plot_normalized_medals(df, metric_col, metric_name, season, year, top_n=10):
    ...
    """
    # Filter to most recent year with medals
    recent = df[df['Year'] == year].copy()
    recent = recent[recent['Total'] > 0]  # Only medal winners
    recent = recent[recent[metric_col].notna()]  # Only with normalization data
    ...
    for i, (val, total) in enumerate(zip(values_top, top['Total'].values)):
        ax_top.text(val + val*0.02, i, f'{val:.2f} ({int(total)} medals)', 
    ...
    # Add values at end of bars
    for i, (val, total) in enumerate(zip(values_bottom, bottom['Total'].values)):
        ax_bottom.text(val + val*0.02, i, f'{val:.2f} ({int(total)} medals)', 
                      va='center', fontsize=9)
...
def main():
    print("="*70)
    print("NORMALIZED MEDALS: TOP & BOTTOM PERFORMERS")
    print("="*70)
    print()
    ...
    # ===== SUMMER: PER CAPITA =====
    print(f"Summer {summer_year}: Medals per million population")
    print(f"  Total countries with data: {len(summer[(summer.Year==summer_year) & (summer.Medals_per_million_pop.notna()) & (summer.Total > 0)])}")
    ...
    if fig_top:
        save_plot(fig_top, PLOTS_DIR / f'summer_{summer_year}_per_capita_TOP_{TOP_N}.png')
        plt.close(fig_top)
        print(f"  ✓ Saved: summer_{summer_year}_per_capita_TOP_{TOP_N}.png")
    
    if fig_bottom:
        save_plot(fig_bottom, PLOTS_DIR / f'summer_{summer_year}_per_capita_BOTTOM_{TOP_N}.png')
        plt.close(fig_bottom)
        print(f"  ✓ Saved: summer_{summer_year}_per_capita_BOTTOM_{TOP_N}.png")
    
    print()
    
    # ===== WINTER: PER CAPITA =====
    print(f"Winter {winter_year}: Medals per million population")
    print(f"  Total countries with data: {len(winter[(winter.Year==winter_year) & (winter.Medals_per_million_pop.notna()) & (winter.Total > 0)])}")
    ...
    if fig_top:
        save_plot(fig_top, PLOTS_DIR / f'winter_{winter_year}_per_capita_TOP_{TOP_N}.png')
        plt.close(fig_top)
        print(f"  ✓ Saved: winter_{winter_year}_per_capita_TOP_{TOP_N}.png")
    
    if fig_bottom:
        save_plot(fig_bottom, PLOTS_DIR / f'winter_{winter_year}_per_capita_BOTTOM_{TOP_N}.png')
        plt.close(fig_bottom)
        print(f"  ✓ Saved: winter_{winter_year}_per_capita_BOTTOM_{TOP_N}.png")
