"""
Plot medals per GDP trends over time.

Shows line plots of the top countries by medals per billion USD GDP
...
def plot_per_gdp_trends():
    """Create line plots showing per GDP medal trends over time."""
    ...
    for season, df in [('Summer', summer_df), ('Winter', winter_df)]:
        ...
        ax.set_ylabel('Medals per Billion USD GDP', fontsize=16, weight='bold')
        ax.set_title(
            f"Top {TOP_N_COUNTRIES} Countries: Medals per GDP Over Time\n"
        ...
        # Save top plot
        filename = f'{season.lower()}_per_GDP_trends_top_{TOP_N_COUNTRIES}.png'
        ...
        ax.set_ylabel('Medals per Billion USD GDP', fontsize=16, weight='bold')
        ax.set_title(
            f"Bottom {TOP_N_COUNTRIES} Countries: Medals per GDP Over Time\n"
            f"{season} Olympics ({START_YEAR}-{END_YEAR})",
            fontsize=18,
            weight='bold',
            pad=20
        ...
        # Save bottom plot
        filename = f'{season.lower()}_per_GDP_trends_bottom_{TOP_N_COUNTRIES}.png'
        output_file = PLOTS_DIR / filename
        save_plot(fig, output_file)
        plt.close()
        print(f"✓ Saved: {output_file}")
