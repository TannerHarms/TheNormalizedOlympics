"""
Plot medals per capita trends over time.

Shows line plots of the top countries by medals per million population
...
def plot_per_capita_trends():
    """Create line plots showing per capita medal trends over time."""
    ...
    for season, df in [('Summer', summer_df), ('Winter', winter_df)]:
        ...
        # Filter to countries with population data
        df_filtered = df_filtered[df_filtered['Population'].notna()].copy()
        
        # Calculate per capita metric
        df_filtered['Medals_Per_Million'] = (
            df_filtered['Total'] / df_filtered['Population'] * 1_000_000
        )
        ...
        # Find top and bottom countries by average per capita performance
        country_avg = df_recent.groupby('Country')['Medals_Per_Million'].mean()
        ...
        print(f"Top {TOP_N_COUNTRIES} countries by avg medals per capita:")
        ...
        for idx, country in enumerate(top_countries):
            ...
            if len(country_data) > 0:
                ...
                ax.plot(
                    country_data['Year'],
                    country_data['Medals_Per_Million'],
                    marker='o',
                    linewidth=2.5,
                    markersize=8,
        ...
        ax.set_ylabel('Medals per Million People', fontsize=16, weight='bold')
        ax.set_title(
            f"Top {TOP_N_COUNTRIES} Countries: Medals per Capita Over Time\n"
        ...
        # Save top plot
        filename = f'{season.lower()}_per_capita_trends_top_{TOP_N_COUNTRIES}.png'
        ...
        ax.set_ylabel('Medals per Million People', fontsize=16, weight='bold')
        ax.set_title(
            f"Bottom {TOP_N_COUNTRIES} Countries: Medals per Capita Over Time\n"
            f"{season} Olympics ({START_YEAR}-{END_YEAR})",
            fontsize=18,
            weight='bold',
            pad=20
        ...
        # Save bottom plot
        filename = f'{season.lower()}_per_capita_trends_bottom_{TOP_N_COUNTRIES}.png'
        output_file = PLOTS_DIR / filename
        save_plot(fig, output_file)
        plt.close()
        print(f"✓ Saved: {output_file}")
