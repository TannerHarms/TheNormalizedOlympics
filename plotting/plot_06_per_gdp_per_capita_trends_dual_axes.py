"""
Plot medals per GDP per capita trends over time with dual y-axes.
...
TOP_N_COUNTRIES = 6  # Number of top/bottom countries to show
...
def plot_per_gdp_per_capita_trends_dual():
    """Create line plots showing per GDP per capita medal trends with dual y-axes."""
    ...
    for season, df in [('Summer', summer_df), ('Winter', winter_df)]:
        ...
        ax.set_ylabel('Top Performers - Medals per $1000 GDP per Capita', fontsize=16, weight='bold')
        ax2.set_ylabel('Bottom Performers - Medals per $1000 GDP per Capita', fontsize=16, weight='bold')
        ax.set_title(
            f"Top & Bottom {TOP_N_COUNTRIES} Countries: Medals per GDP per Capita Over Time\n"
        ...
        # Save plot
        filename = f'{season.lower()}_per_gdp_per_capita_trends_dual_axes_{TOP_N_COUNTRIES}.png'
...
if __name__ == '__main__':
    plot_per_gdp_per_capita_trends_dual()
